  ...
  return (
    <View style={styles.root}>
      ...
      <View style={styles.field}>
        ...
        <TextInput autoCapitalize="none" keyboardType="email-address" placeholder="you@example.com" placeholderTextColor={MUTED} value={email} onChangeText={setEmail} style={styles.input} />
      </View>
      <Spacer h={12} />
      <View style={styles.field}>
        <Text style={styles.label}>Password</Text>
        <TextInput secureTextEntry placeholder="••••••••" placeholderTextColor={MUTED} value={password} onChangeText={setPassword} style={styles.input} />
      </View>
      <Spacer h={20} />
      <TouchableOpacity style={styles.cta} onPress={onSubmit} disabled={busy}>
        {busy ? <ActivityIndicator /> : <Text style={styles.ctaText}>{mode === 'signIn' ? 'Sign In' : 'Sign Up'}</Text>}
      </TouchableOpacity>
      <Spacer h={14} />
      <TouchableOpacity onPress={() => setMode(mode === 'signIn' ? 'signUp' : 'signIn')}>
        <Text style={styles.link}>{mode === 'signIn' ? "Don't have an account? Sign Up" : 'Have an account? Sign In'}</Text>
      </TouchableOpacity>
      {msg ? (<><Spacer h={16} /><Text style={styles.msg}>{msg}</Text></>) : null}
    </View>
  );
};
...
const styles = StyleSheet.create({
  ...
  input: { borderWidth: 1, borderColor: BORDER, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, color: FG, backgroundColor: '#1a1a1a' },
  cta: { backgroundColor: ORANGE, paddingVertical: 14, borderRadius: 14, alignItems: 'center' },
