...
// UPDATED:
// 1) Back button (top-left) uses router.back()
// 2) Integrates FromRecipePopup (select from saved meals)
// 3) Keeps created/added meals living **below MacroSection** per selected day
...
export default function AddMealScreen() {
  ...
  const addMealToToday = (meal: MealData) => {
    setMealsByDay(prev => ({ ...prev, [selectedIndex]: [meal, ...(prev[selectedIndex] ?? [])] }));
  };

  const addManyMealsToToday = (meals: MealData[]) => {
    if (meals.length === 0) return;
    setMealsByDay(prev => ({ ...prev, [selectedIndex]: [...meals, ...(prev[selectedIndex] ?? [])] }));
  };
          ...
          <View style={{ marginTop: 10 }}>
            {todaysMeals.map(m => (
              <View key={m.id} style={{ marginBottom: 12 }}>
                ...
                <View style={[styles.hr, { marginTop: 10, opacity: 0.6 }]} />
              </View>
            ))}
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  empty: { color: C.muted, marginTop: 10, alignSelf: 'center' },
