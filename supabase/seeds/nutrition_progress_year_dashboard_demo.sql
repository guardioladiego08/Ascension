...
-- Generates deterministic nutrition history spanning 56 weeks for one user.
...
-- 4) Open Progress -> Nutrition to test intake trends, adherence, timing, repeat behavior,
...
begin
  ...
  loop
    ...
    from (
      select
        ...
        c.ordinal_position,
        case
          when c.column_name = 'id' then 'f.id'
  ...
  if v_has_favorite_foods then
    ...
    values
      (v_user_id, v_food_banana),
      (v_user_id, v_food_protein_bar),
      (v_user_id, v_food_greek_yogurt)
  ...
  if not v_replace_existing_days then
    ...
    delete from nutrition.diary_days d
    using tmp_nutrition_seed_days s
    where d.user_id = v_user_id
      and d.date = s.diary_date;

    get diagnostics v_replaced_days = row_count;
  end if;
  ...
  raise notice
    'Nutrition progress demo ready for user %: % foods/recipes refreshed, % seeded days generated, % existing days matched, % days replaced, % days inserted, % diary items inserted, range % -> %.',
    v_user_id,
    20,
    v_seeded_days,
    v_existing_days,
